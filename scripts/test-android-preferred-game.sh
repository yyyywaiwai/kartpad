...
out="$repo/.android-bootstrap/preferred-game-host-tests"
mkdir -p "$out"
"$jdk/bin/java" -cp "$compiler:$stdlib:$annotations:$coroutines:$reflect" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  ...
  "$repo/android/app/src/main/java/dev/kartpad/android/KartPadPreferredGame.kt" \
  "$repo/tests/android_identity/AtomicFile.kt" \
  "$repo/tests/android_identity/Os.kt" \
  "$repo/tests/android_preferred_game/PreferredGameTests.kt"
"$jdk/bin/java" -cp "$out/tests.jar:$stdlib" dev.kartpad.android.PreferredGameTestsKt
