...
"$adb" shell am start -W -n dev.kartpad.android/.KartPadActivity \
  --ez dev.kartpad.android.TEST_TOUCH_MODAL_CLEAR true >/dev/null

expected="A4 modal clear fixture passed held=0x10 owners=1 neutral=0x0 owners=0"
for _ in {1..30}; do
  ...
  if grep -Fq "A4 modal clear fixture failed" <<<"$output"; then
    printf '%s\n' "$output" >&2
    exit 1
  fi
  sleep 1
...
echo "ERROR: modal-clear fixture did not emit its pass marker" >&2
