...
out="$repo/.android-bootstrap/health-host-tests"
mkdir -p "$out"
"$jdk/bin/java" -cp "$compiler:$stdlib:$annotations:$coroutines:$reflect" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  ...
  "$repo/android/app/src/main/java/dev/kartpad/android/KartPadHealthJournal.kt" \
  "$repo/tests/android_health/HealthJournalTests.kt"
"$jdk/bin/java" -cp "$out/tests.jar:$stdlib" dev.kartpad.android.HealthJournalTestsKt
