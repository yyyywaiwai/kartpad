...
"$adb" shell am start -W -n dev.kartpad.android/.KartPadActivity \
  --ez dev.kartpad.android.TEST_TOUCH_LIFECYCLE_CLEAR true >/dev/null

armed="A4 lifecycle clear fixture armed held=0x10 owners=1"
for _ in {1..30}; do
  output="$("$adb" logcat -d -v brief KartPadFixture:I AndroidRuntime:E '*:S')"
  if grep -Fq "$armed" <<<"$output"; then break; fi
  if grep -Fq "A4 lifecycle clear fixture failed" <<<"$output"; then
    printf '%s\n' "$output" >&2
    exit 1
  fi
  sleep 1
done
grep -Fq "$armed" <<<"${output:-}" || {
  echo "ERROR: lifecycle-clear fixture did not arm a held touch" >&2
  exit 1
}
...
for _ in {1..30}; do
  output="$("$adb" logcat -d -v brief KartPadFixture:I AndroidRuntime:E '*:S')"
  if grep -Fq "A4 lifecycle clear fixture passed reason=" <<<"$output" &&
      grep -Fq "neutral=0x0 owners=0" <<<"$output"; then
    printf '%s\n' "$armed"
    grep -F "A4 lifecycle clear fixture passed reason=" <<<"$output" | tail -1
    exit 0
  fi
  if grep -Fq "A4 lifecycle clear fixture failed" <<<"$output"; then
    printf '%s\n' "$output" >&2
...
echo "ERROR: lifecycle-clear fixture did not emit its pass marker" >&2
