...
def restore(metadata, root):
    ...
    # Rebuild children first, using the exact gitlink identities in the manifest.
    for entry in metadata["entries"]:
        if entry.get("submodule"):
            child = root / entry["path"]
            child.mkdir(parents=True, exist_ok=True)
            if "source" in entry:
                if entry["source"]["commit"] != entry["gitObject"]:
                    raise SystemExit(f"Submodule identity mismatch: {entry['path']}")
                restore(entry["source"], child)
            continue
...
if __name__ == "__main__":
    ...
    restore(metadata, metadata_path.parent.parent / "source" / metadata["label"])
